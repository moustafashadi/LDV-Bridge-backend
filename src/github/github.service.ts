...
export class GitHubService {
  ...
  async createSandboxBranch(sandbox: Sandbox): Promise<GitHubBranch> {
    ...
    // Use the pre-sanitized githubBranch if available, otherwise sanitize sandbox.name
    const branchName =
      sandbox.githubBranch ||
      `sandbox/${sandbox.name
        .toLowerCase()
        .replace(/[^a-z0-9-]/g, '-') // Replace any non-alphanumeric (except -) with -
  ...
  private async createTreeFromDirectory(
    ...
    // Add LDV-Bridge CI/CD workflow file to the tree
    // This ensures sandbox branches trigger validation pipelines
    const workflowContent = `# LCNC Validation Pipeline
# Auto-generated by LDV-Bridge
# This workflow runs automated checks on sandbox branches to validate citizen developer changes
# Results are reported back to LDV-Bridge via webhook

name: LCNC Validation

on:
  push:
    branches:
      - "sandbox/**" # Triggers on any sandbox branch
  workflow_dispatch:
    inputs:
      changeId:
        description: "LDV-Bridge Change ID for tracking"
        required: false
        type: string

env:
  LDV_BRIDGE_WEBHOOK_URL: \${{ secrets.LDV_BRIDGE_WEBHOOK_URL }}
  CICD_WEBHOOK_SECRET: \${{ secrets.CICD_WEBHOOK_SECRET }}

jobs:
  validate:
    name: Validate LCNC Changes
    ...
    steps:
      ...
      - name: Extract Change ID
        id: extract_change
        run: |
          if [ -n "\${{ inputs.changeId }}" ]; then
            echo "change_id=\${{ inputs.changeId }}" >> \$GITHUB_OUTPUT
            exit 0
          fi
          COMMIT_MSG="\${{ github.event.head_commit.message }}"
          CHANGE_ID=\$(echo "\$COMMIT_MSG" | grep -oP '\\[change:\\K[a-f0-9-]+(?=\\])' || echo "")
          echo "change_id=\$CHANGE_ID" >> \$GITHUB_OUTPUT

      - name: Report Pipeline Started
        if: env.LDV_BRIDGE_WEBHOOK_URL != '' && steps.extract_change.outputs.change_id != ''
        run: |
          curl -X POST "\${{ env.LDV_BRIDGE_WEBHOOK_URL }}/cicd/webhook" \\
            -H "Content-Type: application/json" \\
            -H "x-webhook-secret: \${{ env.CICD_WEBHOOK_SECRET }}" \\
            -d '{
              "changeId": "'\${{ steps.extract_change.outputs.change_id }}'",
              "status": "running",
              "runId": "'\${{ github.run_id }}'",
...
`;
    ...
    treeItems.push({
      path: '.github/workflows/lcnc-validation.yml',
      mode: '100644',
      type: 'blob',
    ...
    this.logger.log(
      `[TREE] Processing ${textFileCount} text files and ${binaryFileCount} binary files (includes workflow)`,
    );
